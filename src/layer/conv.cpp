...
void Conv::backward(const Matrix& data_input, const Matrix& grad_input) {
    int n_sample = data_input.cols();
    grad_weight.setZero();
    grad_bias.setZero();
    ...
    for (int i = 0; i < n_sample; i ++) {
        // im2col of grad_top
        Matrix grad_input_i = grad_input.col(i);
